...
- **历史证据（旧版本；Minimax 现已禁用）——混合模型验证局（hy3 ×7 + minimax-m3:free ×2，2026-08-29）**：9 人标准局完整打完（警长竞选→3 天放逐/夜晚→女巫放逐时证伪 2 条带消息引用的身份欺骗→夜刀猎人→警徽移交→猎人压枪→狼人阵营胜）。同时是 Phase 3 四项社会引擎机制的首次真实对局验证：信念融合（5 agent 共 31 条已裁决信念，Brier 0.159–0.227，全部优于 0.25 的抛硬币基线）、评价关系调制、情绪镜像驱动的讨论人格、压缩 pinned 反思（上下文压力 4 normal + 3 hard-guard、峰值 1.544——压缩发生后最近反思仍被逐字钉死）。该记录仅用于说明旧版行为，不能作为当前模型兼容性或可参赛状态的证据。对局证据：`artifacts/transcripts/werewolf-mixed-validation-2026-08-29.json`。
...
- **历史证据（旧版本；Minimax 现已禁用）——狼队合谋轮验证局（hy3 ×7 + minimax-m3:free ×2，2026-08-30）**：村庄胜（2 天终局）：d1 放逐猎人开枪带走一狼、当夜狼刀女巫而女巫同夜毒掉一狼，d2 放逐末狼。合谋轮实证：夜晚狼队团队频道出现 5 条真实协调发言（"夜里刀谁你定，我兜底"→"落刀定了：刀许衡"），实际夜刀目标与合谋一致（许衡/女巫）；两夜提名一致未触发刀型协调轮（分裂路径由契约测试覆盖）。该记录只保留为旧版机制证据；当前随机池不包含 Minimax，当前可用性以 `enabled + protocol passed` 为准。证据：`artifacts/transcripts/werewolf-packpact-validation-2026-08-30.json`（含全知投影与团队频道发言）。
