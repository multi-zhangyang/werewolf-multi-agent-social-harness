...
| --- | --- | --- | --- | --- | --- | --- |
| 狼人杀 | ✅ 49 项（重清点：本文件狼人杀专属用例；含 §28 两项、遗言、平票 PK、警长竞选/警徽流、压枪级联、工具集快照契约、狼队合谋轮与刀型协调） | ✅ 投票密封 | ❌ 无 | ✅ 提取/显式主张均对账 | ✅ ×10 | ✅ 10 |
| 囚徒困境 | ✅ 11 项 | ✅ 不越权断言 | ✅ `choose-move` | ✅ | ✅ | ✅ 10 |
...
| 信任博弈 | ✅ 16 项 | —（顺序行动） | ✅ `return-at-least`/`invest-at-least`/`return-ratio` | ✅ | ✅ | ✅ 4 |
...
- **混合模型验证局（hy3 ×7 + minimax-m3:free ×2 随机发牌，2026-08-29）**：9 人标准局完整打完（警长竞选→3 天放逐/夜晚→女巫放逐时证伪 2 条带消息引用的身份欺骗→夜刀猎人→警徽移交→猎人压枪→狼人阵营胜）。同时是 Phase 3 四项社会引擎机制的首次真实对局验证：信念融合（5 agent 共 31 条已裁决信念，Brier 0.159–0.227，全部优于 0.25 的抛硬币基线）、评价关系调制、情绪镜像驱动的讨论人格、压缩 pinned 反思（上下文压力 4 normal + 3 hard-guard、峰值 1.544——压缩发生后最近反思仍被逐字钉死）。模型兼容性：minimax（思考但不返回思考内容）两席全程零兼容异常，与 hy3（返回思考内容）混排无任何特殊处理。终局零 notice 码、零弃置回合，8 次重试全部当场吸收（4 次操作员 resume），延迟 p50 34s / p95 124s，130 个模型回合。对局证据：`artifacts/transcripts/werewolf-mixed-validation-2026-08-29.json`。
- **狼队合谋轮与刀型协调 / 欺骗行为生效链（2026-08-30）**：夜晚重构为狼队子轮——≥2 狼时先开团队频道合谋轮（`ww:N:night-pact`，期间提名被 `NIGHT_TARGET_EARLY` 拒绝），提名轮后若提名分裂（无严格多数）开放恰好一轮刀型协调（`ww:N:night-realign`，允许换提名），此后 plurality 落刀、字典序仅作有界兜底；独狼跳过仪式。3 项契约测试（提前提名拒绝、分裂→协调→锁定、独狼直定）。同批接通 `inducedActionReceipts`：账本 `linkOutcomeToDeceptions` 在已"被信"的欺骗 episode 上，把其受众（未转为拒绝者）后续落账的行动凭证记为 induced 并推进 `behaviorally-effective`（语义刻意收窄为"先信后行"，检测仍覆盖梯子）——2 项账本测试钉住"结算→引用→只链一次"与"拒绝后不再计数"。证据：`tests/contract/werewolf-rules.test.ts` 与 `tests/unit/social-ledger.test.ts`。
- **狼队合谋轮验证局（hy3 ×7 + minimax-m3:free ×2 随机发牌，2026-08-30）**：村庄胜（2 天终局）：d1 放逐猎人开枪带走一狼、当夜狼刀女巫而女巫同夜毒掉一狼，d2 放逐末狼。合谋轮实证：夜晚狼队团队频道出现 5 条真实协调发言（"夜里刀谁你定，我兜底"→"落刀定了：刀许衡"），实际夜刀目标与合谋一致（许衡/女巫）；两夜提名一致未触发刀型协调轮（分裂路径由契约测试覆盖）。终局零 notice 码、零弃置回合，8 次重试当场吸收，延迟 p50 39s / p95 128s，104 个模型回合。欺骗 1 条被识破、未达 behaviorally-effective（该链路由账本测试钉住）。证据：`artifacts/transcripts/werewolf-packpact-validation-2026-08-30.json`（含全知投影与团队频道发言）。本局模型池已收敛为 hy3 + minimax-m3:free 两档案。
...
- 重跑审计时同步更新顶部基线提交与日期，并核对"真实对局"与 `artifacts/transcripts/` 实际文件数一致（`-metrics` 附加文件不计入；同一房间的中途快照导出以终局记录为准，避免重复计局）。