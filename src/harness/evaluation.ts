...
export interface HarnessEvaluationContext<TState = unknown, TMetrics = unknown, TSocialEpisode = unknown> {
  id: string;
...
export interface HarnessEvaluator<TState = unknown, TMetrics = unknown, TSocialEpisode = unknown, TOutput = unknown> {
  id: string;
  label: string;
  version: string;
  manifest?: HarnessEvaluatorManifestConfig;
  evaluate(context: HarnessEvaluationContext<TState, TMetrics, TSocialEpisode>): HarnessEvaluationModuleResult<TOutput>;
}
...
export function runEvaluationRegistry<TState, TMetrics, TSocialEpisode>(options: {
  id: string;
  context: HarnessEvaluationContext<TState, TMetrics, TSocialEpisode>;
  evaluators: Array<HarnessEvaluator<TState, TMetrics, TSocialEpisode>>;
  createdAt?: string;
}): HarnessEvaluationReport {
  ...
  return {
    ...
    warnings: collectEvaluationWarnings(moduleRuns),
...
export function summarizeMetrics(metrics: HarnessMetricRecord[]): HarnessEvaluationReport["summary"] {
  const scoreMetrics = metrics.filter((metric) => isFiniteNumber(metric.value) && isFiniteNumber(metric.weight) && metric.weight > 0);
  return {
...
function collectEvaluationWarnings(moduleRuns: EvaluationModuleRun[]): HarnessEvaluationWarning[] {
  ...
  for (const moduleRun of moduleRuns) {
    ...
    for (const item of metrics) {
      warnings.push(...metricWarnings(item, result));
    }
  }
  return warnings;
}
...
function metricWarnings(metric: HarnessMetricRecord, result: HarnessEvaluationModuleResult): HarnessEvaluationWarning[] {
  const warnings: HarnessEvaluationWarning[] = [];
