/**
 * Stag-hunt mid-state checkpoint round-trips (AGENTS.md §26).
 */
...
function makeWorld(): SocialWorldBase {
  const world = createWorld({ roomId: "r-shr", scenarioId: "stag-hunt", profiles, rounds: 2 }) as SocialWorldBase;
  world.start();
  return world;
}

function restore(state: WorldSerializedState): SocialWorldBase {
  const world = createWorld({ roomId: "r-shr", scenarioId: "stag-hunt", profiles, rounds: 2, state }) as SocialWorldBase;
  world.start();
  return world;
}
...
it("the sealed choice with one side committed resumes and settles", async () => {
  ...
  assert.equal(scores[P1], 4, "both stags earn 4 each");
