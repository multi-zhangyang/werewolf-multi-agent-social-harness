...
Society 让多个持续存在、彼此独立、拥有有限信息的模型 Agent，在隐藏身份、承诺、资源冲突、重复互动和群体压力中自主交流与行动。游戏提供社会压力，产品关注的是完整链路：Agent 看到了什么、提出了什么主张、相信了什么、通过工具做了什么，以及世界结果如何改变后续关系。

## 当前可验证能力

- **一名参与者，一个持续主体**：每个席位拥有独立 SDK Agent、持久会话、心智和模型绑定；没有中央模型代演全桌，也没有拥有独立社会身份的 planner/critic 子 Agent。
...
- **稳定人物身份**：人物以永久 `characterId` 绑定跨局关系与赛季会话（同一角色跨局复用同一 SDK Session，模型凭自身上下文记得既往对局，无任何检索/评分式记忆系统）；当前座位、显示名、游戏角色与模型都不是人物主键。
...
创建房间时模型分配有三种模式：统一模型（全部席位同一档案）、逐席配置（每席单独挑选，未选的继承统一选择）、随机混合（勾选模型池后平衡洗牌，提交前以「最终阵容预览」明示每席所得，不做服务端暗箱随机）。同一人物切换模型后仍保留 `characterId`、会话语义、关系、当前游戏角色和公开历史。
