...
面向多 Agent 对抗与社会实验的可复现运行时：用统一实验规格编排有状态 Agent，
在隐藏信息环境中记录每次观察、消息、决策与状态变化，并产出可重放、可分叉、
可评测的实验工件。
...
<p align="center">
  <a href="docs/assets/cockpit-overview.png">
    <img src="docs/assets/cockpit-overview.png" alt="多 Agent Harness Cockpit，展示运行注册表、原生执行指标、脱敏工件上下文与证据检查器" width="100%">
  </a>
  <br>
  <sub>服务端记录 · postgame-redacted · deterministic fixture · 无外部 Provider 调用 · 4 步边界为有意截断</sub>
</p>
...
| 场景 | Harness 提供的基础能力 |
| --- | --- |
| 多 Agent 对抗与社会行为研究 | 隐藏信息、分级通信、欺骗、说服、联盟、声誉和关系证据 |
| Agent 策略与模型评测 | 固定 seed、profile/model assignment、生命周期完整的指标与锦标赛 |
| Agent Runtime 回归测试 | 类型化动作、确定性环境、失败记录、状态 hash 和无模型重放 |
| 实验复现与反事实分析 | 原生工件、checkpoint、fork 谱系、矩阵实验和证据引用 |
| 新社会环境接入 | 领域无关的调度、观察、通信、Agent scaffold、评测和工件合同 |
...
- **类型化环境权威**：模型与策略只能提出候选；Harness 校验调度与消息边界，
  最终只有确定性环境能够接受类型化命令并提交状态转换。
- **原生执行工件**：每个系统与 Actor 步骤都记录提交状态、前后 hash、消息范围、
  receipt 和证据引用；顺序步骤与并行批次均可审计。
- **无模型重放**：Replay 只消费已记录的原生步骤，不调用 Agent、策略、
  reasoner 或 provider。
- **Checkpoint 与 Fork 谱系**：经过验证的执行边界可以创建新分支，同时严格
  区分 deterministic replay 与重新执行。
- **规模化评测**：只有真正完成并产生终局结果的 episode 进入胜率与排行榜；
  truncated 和 failed 运行仍保留在成本、延迟、错误和步骤遥测中。
...
<p align="center">
  <a href="docs/assets/werewolf-review.png">
    <img src="docs/assets/werewolf-review.png" alt="狼人杀赛后复盘工作区，展示九个座位、赛后角色、原生步骤统计与脱敏边界" width="100%">
  </a>
  <br>
  <sub>九人隐藏信息局面的赛后角色、服务端工件状态与安全投影；fixture 在 4 个原生步骤后有意截断</sub>
</p>

### 社会证据

<p align="center">
  <a href="docs/assets/social-evidence.png">
    <img src="docs/assets/social-evidence.png" alt="社会证据工作区，展示九个 Actor、已记录消息、通信通道、journal 统计与服务端投影图" width="100%">
  </a>
  <br>
  <sub>节点、消息与可见性证据来自服务端投影工件，不由浏览器推断；短 fixture 尚未形成 observed-exposure 边</sub>
</p>
...
### 无 API Key 体验 Cockpit

仓库内置确定性 fixture。它运行真实的 production React build 和 Harness/API
链路，但不会连接外部模型 Provider：

```bash
npm run build
npm run e2e:fixture-server
```

打开 <http://127.0.0.1:4173> 即可浏览运行注册表、狼人杀复盘、原生时间线、
社会证据和评测工作区。体验结束后在启动终端按 `Ctrl+C` 关闭服务。

### 连接模型 Provider

复制环境变量模板，按[模型 Provider 配置](#模型-provider-配置)填写协议、Endpoint、
模型 ID 与本地密钥，然后启动开发环境：
...
持久化、重放、评测编排、工件发布与安全投影契约保持领域无关。

详细设计文档：

| 文档 | 内容 |
| --- | --- |
| [系统架构](docs/architecture.md) | 运行权威、模块边界、工件、重放、checkpoint 与 fork |
| [社会 Harness](docs/social-harness.md) | Agent scaffold、通信、社会状态、观察范围与评测合同 |
| [Harness 技术研究](docs/harness-research.md) | 外部系统对照、采用原则与本地架构结论 |
| [设计路线图](docs/multi-agent-society-harness-plan.md) | 能力路线、验收边界与后续演进方向 |

## 模型 Provider 配置

建议显式设置 `LLM_PROVIDER_PROTOCOL`；未设置时默认使用
`openai-chat-completions`。模型 ID 始终是透明传递的配置值，不会触发模型专属
...
真实 Chat Completions 调用使用 streaming。OpenAI-compatible Chat Completions
adapter 不发送 `max_tokens`、`max_completion_tokens` 或等价 token 限制字段；
Anthropic Messages 按协议合同要求显式配置并发送 `max_tokens`。瞬时重试保持
...
运行可复现锦标赛。该示例规格中的 `model-wolf`、`model-village-a` 和
`model-village-b` 是占位模型 ID，运行前应替换为 Provider 实际支持的模型：
...
当规格文件显式定义 `profiles` 时，profile 内的模型优先于单独的 `--models`；
需要修改这类规格的模型时，应编辑 `profiles` 或使用 `--profiles` 覆盖。
[experiments](experiments/) 目录提供可执行示例和需要填入模型 ID 的实验模板。
...
Express 服务与 CLI 共享同一套 Harness 执行、工件、重放和评测权威，并提供
面向 Cockpit 的安全 HTTP 入口：
...
| `POST /api/matches/:id/replay` | 校验和检查无模型重放 |
| `POST /api/matches/:id/checkpoints` | 在已验证的原生步骤边界创建 checkpoint |
| `POST /api/checkpoints/:id/fork` | 从 canonical checkpoint 创建新执行谱系 |
...
当前项目以源码形式提供，尚未作为 npm 包发布。公共 TypeScript/API 契约应以
所使用的仓库版本及对应架构文档为准。
...
欢迎提交贡献。功能建议和普通缺陷可通过 [GitHub Issues](https://github.com/multi-zhangyang/werewolf-multi-agent-social-harness/issues)
反馈；提交代码前请阅读 [CONTRIBUTING.md](CONTRIBUTING.md)，了解开发流程与
