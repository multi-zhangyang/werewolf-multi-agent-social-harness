...
- **一切实时可见** —— 思考模型的隐藏推理、每个 Agent 自己产出的结构化 ThoughtBeat、每一次工具调用、每一条公聊与密谋，都像直播一样流向观察席。身份揭晓与淘汰，是戏剧性时刻而不是控制台日志。
...
| 世界 | 核心张力 |
| --- | --- |
| 狼人杀 | 6/8/9/10/12 人官方板子：狼人·狼王·预言家·女巫·猎人·守卫·小丑·村民，解药毒药、奶穿与临死开枪全部真实结算 |
| 阿瓦隆 | 5-10 人官方配置：忠臣与内奸混坐圆桌，任务成败藏于一次举手，以及最终的梅林刺杀 |
...
```text
浏览器
  ▲ SSE 快照与实时事件（状态 · 隐藏推理 · ThoughtBeat · 发言 · 世界变更）
  │
...
| POST | `/api/rooms` | 创建并启动房间（支持统一模型、轮转、单 Agent 覆盖与参数覆盖） |
